...
# Get allowed origin from environment or default to * for development
...
# Check if the RAPID_KEY is set, otherwise use the hardcoded key if present
RAPID_KEY = os.getenv("RAPID_KEY", "291bb8783bmsha25eed8621bfae3p1f61ddjsn5d3883db32e9")
RAPID_HOST = os.getenv("RAPID_HOST", "fresh-linkedin-profile-data.p.rapidapi.com")

if not RAPID_KEY:
    print("WARNING: RAPID_KEY environment variable not set. API calls will fail.")
...
async def enrich_profile(url: str, client: httpx.AsyncClient) -> dict:
    ...
    try:
        ...
        api_url = f"https://{RAPID_HOST}/get-profile"
...
async def enrich_single_profile(data: Dict[str, Any]):
    """Directly enrich a single profile without creating a job - ideal for Bubble integration"""
    try:
        ...
        # Enrich the profile directly
        async with httpx.AsyncClient() as client:
            enriched_data = await enrich_profile_with_original(linkedin_url, data, client)
